...
\title{\textbf{面向 LLM 社会模拟的科学性纪律：\\以独居老人社会支持干预为案例的分层生成式智能体模拟}}
...
\small
多智能体 LLM 社会模拟面临三重质疑：数据是否凭空捏造、结论是否可复现、发现是否只是规则的回声。本文以独居老人社会支持干预为案例，提出并演示一套面向 LLM 社会模拟的\textbf{科学性纪律}：预注册假设与效度门槛、真实调查数据锚定、失效 run 归档与根因分类、含沉没成本的成本核算，以及\textbf{``规定 vs 涌现''的逐条对照}。模拟基于 AgentSociety2：6 名 LLM 驱动的焦点老人嵌入 20 人规则层支持网络，1 步 = 1 月、共 24 月，比较专业社工个案管理（casework）、时间银行互助（timebank）与数字平台志愿匹配（platform）于第 7--18 月实施、其后撤出 6 个月。结果（\ValidRuns/\TargetRuns\ 有效 run）：干预期个案管理孤立率最低（\IsoIvCasework\% 对基线 \IsoIvNone\%），撤出后反弹 \IsoRebCasework pp；时间银行反弹 \IsoRebTimebank pp。我们如实说明：这一可持续性分化是在显式编码的机制假设（干预即时接触、结对延续概率）下\emph{复现}的预期形态，而非涌现发现；分层拆解显示宏观孤立率的 \FocShareNone\ 来自 LLM 层，且时间银行的``不反弹''主要由规则层贡献，LLM 层仍反弹 \PFocRebTb pp。真正的涌现结果在行为层：三种干预的求助行为签名可区分，个案管理组求助最多、成功率最低；跨模型检验发现 gpt-6-astra 在无干预时更倾向``不行动''、外部行动者来访会拉动其产出决策，这对 LLM 社会模拟的效度门槛设计具有一般含义。所有结果可由公开脚本一键复现。\par\vspace{0.4em}
...
\textbf{必须说明：这不是涌现发现。}两条机制都写在环境规则里——个案管理的情感接触由社工每月来访直接触发，撤出即消失；时间银行结对撤出后以 post\_persist（中点 0.5）概率延续互动，代码注释明示``延续概率是 H4 的关键机制假设''。LLM 焦点老人与规则老人处在同一环境、共用同一指标定义，规则层与 LLM 层因此\emph{不是}两条独立证据，早先版本``跨层互证''的表述在此撤回。H4 的准确表述是：\textbf{在显式编码的机制假设下，模拟复现了预期的可持续性分化，并给出了灵敏度范围}（post\_persist 0.3--0.7 扫描内方向不变；caseload 4--12 不改变反弹）。它的价值在于把``退出后依赖型支持消失、互惠型关系部分存续''这一实务直觉变成可参数化、可反驳的模型，而非证明它为真。最能检验该假设的补充实验是将 post\_persist 置零观察分化是否消失，本文未执行，列为后续工作。

\subsection{分层拆解：LLM 层在宏观结果中的权重}\label{sec:layers}
表~\ref{tab:layers} 从 replay 中拆出 6 名焦点老人与 14 名规则老人各自的窗口均值。三点观察：（1）\textbf{焦点老人的基线孤立率约为规则老人的三倍}（none：\FocIvNone\% 对 \RulIvNone\%），源于第 3.1 节所述接触生成机制差异——LLM 老人不行动即无接触；（2）\textbf{LLM 层主导了宏观效应}：焦点老人只占 30\% 人口，却贡献了基线孤立率的 \FocShareNone；个案管理对焦点层的配对效应（干预期 \PFocIvCw pp、反弹 \PFocRebCw pp）是规则层（\PRulIvCw pp、\PRulRebCw pp）的三至七倍。因此``生成式智能体模拟''并非装饰——但同时意味着主结果对 LLM 行为特征（含静默）高度敏感；（3）\textbf{时间银行的``不反弹''主要是规则层贡献}：规则层配对反弹 \PRulRebTb pp（post\_persist 起作用），而 LLM 层反弹 \PFocRebTb pp，与平台组（\PFocRebPf pp）相当，仅远小于个案管理（\PFocRebCw pp）。故 LLM 层支持的结论是``个案管理反弹远大于其余两种''，而非``时间银行几乎不反弹''。孤独感在两层均方向一致、幅度平缓。

\begin{table}[t]
\centering\small
\caption{分层拆解（窗口均值；孤立率 \%，孤独感 0--1）。焦点 = 6 名 LLM 老人，规则 = 14 名规则层老人；配对差 = 同 seed 下条件 $-$ none。}
\label{tab:layers}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{2}{c}{焦点孤立率} & \multicolumn{2}{c}{规则孤立率} & \multicolumn{2}{c}{孤独感 焦点/规则} \\
条件 & 干预期 & 撤出期 & 干预期 & 撤出期 & 干预期 & 撤出期 \\
\midrule
...
\midrule
\multicolumn{7}{l}{配对差 vs none（pp）：干预期 / 反弹} \\
...
时间银行 LLM 层仍反弹 \PFocRebTb pp & 涌现 & LLM 老人未按结对关系主动互动 \\
...
\textbf{（三）结构性互助的可持续性是待检验假设而非结论。}时间银行的撤出后延续在规则层由 post\_persist 规定，在 LLM 层并未自发出现（焦点老人反弹 \PFocRebTb pp）。因此``个案管理开路、时间银行接续''应作为后续实验假设提出，并以 post\_persist $=0$ 对照检验其是否依赖该参数。
...
\section{局限与伦理}
\textbf{局限。}（1）$n=3$ 种子（平台臂 $n=\NPlatform$）只支持描述性对比与方向判断，不支持显著性与稳定排序；（2）关系衰减机制存在 46\% 存活率地板，慢于文献实证形态；（3）LLM 焦点老人的行为效度依赖模型本身，行为型静默与真实老人的``合法不行动''难以区分，且 10\% 静默门槛按主模型 gpt-5.6-sol 设定，对更倾向``不行动''的模型（gpt-6-astra none 臂两次为 10.7--11.3\%）可能系统性偏严，见第~\ref{sec:robust}~节；（4）校准数据存在口径差（调查波次、独居子样本 vs 全体）；（5）撤出观察窗仅 6 个月；（7）主结局 isolation\_rate 为月度接触快照，被在场型服务直接命中，效应量应以孤独感为准；（8）宏观结果的一半以上由 6 名 LLM 老人贡献，主结果对 LLM 行为特征高度敏感；（6）Ristolainen RCT 的亚组效应存在基线不可比问题\citep{ristolainen2020effects}，个案管理参数依据以 Masi 荟萃为主。
