...
\title{\textbf{Layered Generative-Agent Simulation of Social Support Interventions for Older Adults Living Alone,\\ and a Scientific Discipline for LLM Social Simulation}}
\author{Dongshuai Hui (惠栋帅) \quad Caihou Wang (王才厚) \quad Congyu Li (李聪煜)\\ \small AISS 2026 Challenge Submission}
\date{}
...
\small
We build a layered generative-agent simulation of social support interventions for older adults living alone, and ship with it a transferable \textbf{scientific discipline} for LLM social simulation (preregistration with validity gates, anchoring to real survey data, root-cause classification of failed runs, cost accounting including sunk costs, and an item-by-item ``prescribed vs.\ emergent'' ledger; see the appendix checklist). The simulation, built on AgentSociety2, embeds six LLM-driven focal elders in a 20-person rules-layer support network (1 step = 1 month, 24 months) and compares professional casework, timebank mutual aid, and digital-platform volunteer matching, implemented in months 7--18 and then withdrawn for six months. Results (\ValidRuns/\TargetRuns\ valid runs): casework yields the lowest in-window isolation rate (\IsoIvCasework\% vs.\ a \IsoIvNone\% baseline) and rebounds by \IsoRebCasework pp after withdrawal; the timebank rebounds by \IsoRebTimebank pp. We state plainly that this sustainability divergence is the expected pattern \emph{reproduced} under explicitly encoded mechanism assumptions (immediate contact on intervention, a pairing-persistence probability), not an emergent discovery, and a rules-layer zero control confirms that with post\_persist $=0$ the timebank returns to the same post-withdrawal level as casework; a layer decomposition shows that \FocShareNone\ of the macro isolation rate comes from the LLM layer; the timebank's ``no rebound'' is mainly a rules-layer contribution, with the LLM layer rebounding by \PFocRebTb pp (\PFocActRebTb pp after excluding silent months); and after casework is withdrawn, LLM elders are more isolated (\FocPostCw\%) than those never treated (\FocPostNone\%), a gap that survives the exclusion of silent months. The emergent results lie in the behavioural layer: the three interventions leave distinguishable help-seeking signatures, with casework generating the most requests at the lowest success rate; and a cross-model check finds that gpt-6-astra is more inclined to ``do nothing'' when no intervention is present, while visits from an external actor pull decisions out of it---a finding with general implications for validity-gate design in LLM social simulation. All results reproduce from the released scripts in a single command.\par\vspace{0.4em}
...
\begin{itemize}
  ...
  \item \textbf{timebank --- time-bank mutual aid}: younger-old healthy elders are paired with high-risk elders and exchange services for credits under automatic rules; the pairings are real interpersonal ties and persist after withdrawal with probability post\_persist (midpoint 0.5; rules-layer scan 0--1 including a zero control, Table~\ref{tab:pp}). Parameters draw on the Hong Kong timebank quasi-experiment\citep{lu2024timebank}; no literature point estimate exists for post-withdrawal persistence, so it is treated as a scan.
...
\textbf{H4} (exploratory) casework rebounds more than the timebank after withdrawal---structural mutual aid is more sustainable (preregistered wording reproduced verbatim; Section~\ref{sec:results} explains that this pattern is fixed by explicit mechanism assumptions and is not read as emergent).
...
\textbf{This must be stated plainly: it is not an emergent finding.} Both mechanisms are written into the environment. Casework's emotional contact is triggered directly by the social worker's monthly visit and disappears on withdrawal; timebank pairings continue to interact after withdrawal with probability post\_persist (midpoint 0.5), and the code comment explicitly labels this persistence probability ``the key mechanism assumption for H4''. LLM focal elders and rules-layer elders inhabit the same environment and share the same indicator definition, so the rules layer and the LLM layer are \emph{not} two independent lines of evidence, and agreement between them does not constitute cross-layer corroboration. The accurate statement of H4 is: \textbf{under explicitly encoded mechanism assumptions, the simulation reproduces the expected sustainability divergence and provides its sensitivity range} (caseload 4--12 does not alter the rebound; the post\_persist scan follows). Its value lies in turning the practice intuition ``dependency-type support vanishes on exit, reciprocal ties partly persist'' into a parameterised, refutable model---not in proving it true.

\textbf{The post\_persist $=0$ control (rules layer, \PPSeeds\ seeds).} To test directly whether the divergence is manufactured by this parameter, we scanned post\_persist $\in\{0,0.3,0.5,0.7,1.0\}$ in the rules layer (Table~\ref{tab:pp}). The result has two halves. (1) \textbf{Post-withdrawal level}: at post\_persist $=0$ the timebank's post-withdrawal isolation rate is \PPPostTbZero\%, identical to casework (\PPPostCw\%)---\emph{once pairings do not persist, the timebank has no sustainability advantage whatsoever}; as post\_persist rises to 0.5/0.7 the post-withdrawal level falls to \PPPostTbMid\%/\PPPostTbHi\%, so the advantage is supplied entirely by the parameter. (2) \textbf{Rebound magnitude}: even at post\_persist $=0$ the timebank rebounds less (\PPRebTbZero pp) than casework (\PPRebCw pp; \PPLtCwZero/\PPSeeds\ seeds), but this is arithmetic: the timebank's in-window effect is weaker (\PPIvTb\% vs.\ \PPIvCw\%), leaving less room to rebound, and both return to the same level. \textbf{Comparing sustainability by rebound magnitude therefore systematically favours weak interventions}; the correct quantity is the post-withdrawal level or the residual effect relative to baseline, and our main table (Table~\ref{tab:main}) reports both. A side observation: both interventions' post-withdrawal levels (\PPPostCw\%) lie below never-intervened (\PPPostNone\%), reflecting slowly decaying residual tie strength built during the window---again a direct consequence of $\delta$.

\begin{table}[t]
\centering\footnotesize\setlength{\tabcolsep}{4pt}
\caption{Rules-layer post\_persist scan (\PPSeeds\ seeds; all elders in the rules layer, so absolute levels are not comparable with the LLM batch). Isolation rate as window mean (\%); rebound $=$ post $-$ in-window (pp); last column: seeds in which the timebank rebound is smaller than same-seed casework.}
\label{tab:pp}
\begin{tabular}{lcccc}
\toprule
Config & In-win. & Post & Reb. & Reb.$<$cw \\
\midrule
none & --- & \PPPostNone & --- & --- \\
...
Table~\ref{tab:layers} decomposes the window means of the six focal elders and the fourteen rules-layer elders from the replays. Three observations. (1) \textbf{Focal elders' baseline isolation rate is about three times that of rules-layer elders} (none: \FocIvNone\% vs.\ \RulIvNone\%), a consequence of the contact-generation difference described in Section 3.1---an LLM elder that does not act has no contact. (2) \textbf{The LLM layer dominates the macro effect}: focal elders are 30\% of the population but contribute \FocShareNone\ of the baseline isolation rate, and casework's paired effect on the focal layer (in-window \PFocIvCw pp, rebound \PFocRebCw pp) is three to seven times its effect on the rules layer (\PRulIvCw pp, \PRulRebCw pp). ``Generative-agent simulation'' is therefore not decorative---but it also means the headline results are highly sensitive to LLM behavioural traits, including silence. (3) \textbf{The timebank's ``no rebound'' is mainly a rules-layer contribution}: the rules-layer paired rebound is \PRulRebTb pp (post\_persist at work), whereas the raw LLM-layer rebound is \PFocRebTb pp, comparable to the platform arm (\PFocRebPf pp) and far below casework (\PFocRebCw pp). What the LLM layer supports is thus ``casework rebounds far more than the other two'', not ``the timebank barely rebounds''. Loneliness moves in the same direction in both layers, with gentler magnitudes.

\textbf{Do the focal-layer numbers measure isolation or silence?} The ``excluding silent months'' columns of Table~\ref{tab:layers} count only focal agent-months with at least one decision record (silence rates in valid runs: 4.0--10.0\%) and answer this question. After exclusion, focal isolation rates fall by 1--8 pp in every condition, the ranking is unchanged, and the LLM layer's dominance is unchanged; but the timebank's LLM-layer rebound shrinks from \PFocRebTb pp to \PFocActRebTb pp---most of it came from silent months in the post-withdrawal window---and after exclusion it sits between the rules layer (\PRulRebTb pp) and casework (\PFocActRebCw pp). The corresponding row of Table~\ref{tab:prescribed} uses the silence-excluded figure.

\textbf{After casework is withdrawn, LLM elders are more isolated than those never treated.} This is the starkest number the decomposition exposes: the focal-layer casework isolation rate in the post-withdrawal window is \FocPostCw\%, against \FocPostNone\% for the none arm in the same window; excluding silent months, \FocActPostCw\% vs.\ \FocActPostNone\%, with a paired rebound of \PFocActRebCw pp---the gap narrows but does not vanish, and all three seeds exceed their paired baseline. Two readings: (a) an \emph{overshoot of dependency-type support}---while the worker is present, LLM elders concentrate their help-seeking on the worker (``most requests'', Section~5.4), displacing spontaneous neighbour visits and family calls; after withdrawal they have lost the worker without recovering their prior action pattern, and the behavioural signature ``continuing to request help from a worker who no longer responds'' corroborates this; (b) a \emph{silence artefact}---the three casework runs have silence rates of 6.0--9.3\%, and silence concentrated in the post-withdrawal window would inflate isolation. The overshoot survives the exclusion of silent months, so (b) cannot fully explain it; but with $n=3$ and a six-month post-window we report (a) as an \textbf{exploratory finding awaiting a test}, not a conclusion, while noting that it is stronger than the ``effects vanish when the programme ends'' pattern the Masi meta-analysis warns about---if it existed in real services, casework without an exit plan could be net harmful.

\begin{table}[t]
\centering\small
\caption{Layer decomposition (window means; isolation in \%, loneliness 0--1). Focal = six LLM elders, Rules = fourteen rules-layer elders; ``excl.\ silent'' = only focal agent-months with a decision record; paired difference = condition $-$ none under the same seed.}
\label{tab:layers}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccccccc}
\toprule
 & \multicolumn{2}{c}{Focal isolation} & \multicolumn{2}{c}{Focal, excl.\ silent} & \multicolumn{2}{c}{Rules isolation} & \multicolumn{2}{c}{Loneliness focal/rules} \\
Condition & Window & Post & Window & Post & Window & Post & Window & Post \\
\midrule
...
\midrule
\multicolumn{9}{l}{Paired difference vs.\ none (pp): in-window / rebound} \\
...
\begin{enumerate}
  ...
  \item \textbf{A directional estimate of delay cost}: each month of postponement raises the two-year mean isolation rate by about \DelayCostCasework pp (casework) / \DelayCostTimebank pp (timebank), about \DelayCumCasework\ pp (casework) over 24 months against a \RulesBaselineOverall\% baseline; the platform's total effect is weak, so it is timing-insensitive (\DelayCostPlatform pp). The magnitude is fixed entirely by the rule property ``interventions act immediately'' (Table~\ref{tab:prescribed}) and is offered only as a directional estimate under the rules-layer mechanism;
  \item \textbf{Rebound is dose-independent}: extending casework from 6 to 12 months does not soften the post-withdrawal rebound (\RulesRebCwLo\ to \RulesRebCwHi pp; the independent post\_persist scan measures \PPRebCw pp, so the two scans corroborate each other)---the rebound of dependency-type support is a property of the mechanism, not a symptom of underdosing.
\end{enumerate}
...
Timebank persistence after withdrawal (rules layer) & prescribed & post\_persist param.; at zero the post-withdrawal level equals casework (Table~\ref{tab:pp}) \\
...
Timebank LLM-layer rebound \PFocActRebTb pp (excl.\ silent months) & emergent & pairing partners are written into focal elders' tie ledgers and can be visited / asked for help (tool access verified), but LLM elders rarely do so \\
...
LLM elders more isolated after casework withdrawal than never treated & emergent (to be tested) & layer decomposition; survives excluding silent months \\
...
Measuring our model's tie-survival curves against Burt's yardstick (rules layer, 50 persons $\times$ 48 months) reveals two \textbf{mechanism-level limits}: (1) tie loss is markedly slower than in the literature (4-year survival \TieSurvFourYr\% vs.\ 38.3\% under Burt's fit); (2) there is a survival floor: scanning $\delta$ from 0.06 to \TieFloorDecay, 4-year survival bottoms out at \TieFloor\% (at $\delta=\TieFloorDecay$ it is already \TieFloorYrOne\% after year 1 and flat thereafter), still above Burt's 38.3\%---children with fixed contact schedules and high-frequency neighbours refresh their contact timestamps monthly, never enter the decay branch, and thus no value of $\delta$ can reproduce the sustained decline of Burt's power function (\texttt{tests/scan\_tie\_survival\_floor.py}). Accordingly we \textbf{do not treat tie-loss rate as a calibration target}, only as a sensitivity dimension. One correction attaches to the Burt citation: the paper's ``2.63-year half-life for ties outside the family'' is an arithmetic slip (a constant term left unsubtracted); recomputed from the paper's own equation it is 1.63 years\citep{burt2000decay}. The deep-isolation measure is truncated by the survival floor and therefore enters sensitivity analyses only.
...
\midrule
gpt-5.6-sol & none     & \McSolSilNone\% & \McSolIvNone & \McSolPostNone & \McSolRebNone \\
            & casework & \McSolSilCw\% & \McSolIvCw  & \McSolPostCw & \McSolRebCw \\
gpt-6-astra & none     & \McAstraSilNone\%$^\dagger$ & \McAstraIvNone & \McAstraPostNone & \McAstraRebNone \\
            & casework & \McAstraSilCw\% & \McAstraIvCw  & \McAstraPostCw & \McAstraRebCw \\
\midrule
\multicolumn{2}{l}{Paired casework$-$none: sol}   & & \McSolPIv & \McSolPPost & \McSolPReb \\
\multicolumn{2}{l}{Paired casework$-$none: astra} & & \McAstraPIv & \McAstraPPost & \McAstraPReb \\
\bottomrule
\end{tabular}}
...
\textbf{(3) The sustainability of structural mutual aid is a hypothesis to test, not a conclusion.} The timebank's post-withdrawal persistence is prescribed in the rules layer by post\_persist and did not arise spontaneously in the LLM layer (focal elders rebound by \PFocRebTb pp; \PFocActRebTb pp excluding silent months). The rules-layer zero control (Table~\ref{tab:pp}) confirms that at post\_persist $=0$ the timebank's post-withdrawal level equals casework's: the sustainability advantage depends entirely on that parameter. ``Casework to open, timebank to sustain'' can therefore only be posed as a follow-up hypothesis; whether it holds depends on the real-world persistence rate of pairings after a programme ends, precisely the quantity the existing timebank literature does not measure\citep{lu2024timebank}, and the single parameter in this model most worth calibrating with empirical data.
...
\begin{itemize}
  ...
  \item \textbf{Mechanism scans and loop}: \texttt{tests/scan\_timing.py}, \texttt{tests/scan\_post\_persist.py}, \texttt{tests/scan\_tie\_survival\_floor.py}, \texttt{tools/harness\_loop.py} (preregistered headers + per-round JSONL audit logs).
...
\section{A Transferable Checklist: Scientific Discipline for LLM Social Simulation}\label{app:checklist}
The table below compresses the discipline practised in this paper into checklist items that other LLM social-simulation studies can apply directly, each with the place where this paper implements it.
\begin{table}[H]
\centering\small
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.05\columnwidth}>{\raggedright\arraybackslash}p{0.47\columnwidth}>{\raggedright\arraybackslash}p{0.37\columnwidth}@{}}
\toprule
\# & Item & Where in this paper \\
\midrule
1 & Hypotheses, primary/secondary outcomes, seeds and validity gates written into versioned scripts before the batch & \texttt{run\_batch.sh} header; Section~\ref{sec:prereg} \\
2 & Gates distinguish infrastructure-type from behavioural-type failure and state their dependence on the model & Section 7.4; Section~\ref{sec:robust} \\
3 & Failed runs archived, not deleted, each with a root cause; no ``run until it passes'' & \texttt{*.invalid-*} directories; \texttt{silence\_diagnosis} \texttt{.json} \\
4 & Input distributions anchored dimension by dimension to real surveys, with definitional gaps and uncovered parts stated & Section 3.4 \\
5 & At least one output-side external-validity check, failures reported & Sections 7.1--7.2 (Burt mismatch) \\
6 & Every headline result labelled prescribed / semi-prescribed / emergent, with source & Table~\ref{tab:prescribed} \\
7 & Hybrid architectures decompose each layer's contribution to the macro outcome, with silence-excluded figures & Table~\ref{tab:layers} \\
8 & Nature of indicators stated: snapshot vs.\ dynamic; which one effect sizes are read from & Section~\ref{sec:prereg} \\
9 & Baseline silence rate without intervention reported per model as a selection statistic & Section~\ref{sec:robust} \\
10 & Costs include sunk costs of invalid and comparison runs; call counts reported & Section 8 \\
11 & All numbers in the text generated by script, none transcribed by hand & \texttt{numbers.tex} \\
12 & Autonomous loops have a preregistered search space, round and resource caps, and a per-round audit log & Section 6.2 \\
\bottomrule
\end{tabular}
\end{table}
